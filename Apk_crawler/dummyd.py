...
class ApkPure:
    ...
    def parser_apks(self,count=1):
        ...
        while count:
            wbdata = BeautifulSoup(requests.get("https://apkpure.com/cn/app?sort=download&page="+str(page_num),headers=self.header).text,  'html.parser')
            target_li = wbdata.find_all('li')
            response = urllib.request.urlopen("https://apkpure.com/cn/app?sort=download&page="+str(page_num))
            if response.getcode() == 200: 
                download_links=target_li
                # .find_all('a',attrs={'rel':'nofollow'})['href']
                print(download_links)            
                # download_links=soup.body.contents[9].find_all('a',href=re.compile("/download?"))
            else :
                print("Not working cuz of not 200")
